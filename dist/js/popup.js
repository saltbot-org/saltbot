...
var btnClicked = function (clicktype, data = null) {
    data = data || null;
    chrome.tabs.query({
        active: true,
        currentWindow: true,
...
var multiplierChange = function () {
    const multiplierValue = $("#multiplierSlider")[0].value;
...
Simulator.prototype.evalMutations = function () {
    const self = this;
    chrome.storage.local.get(["characters_v1", "chromosomes_v1"], function (results) {
        return __awaiter(this, void 0, void 0, function* () {
            let matches = [];
            chrome.runtime.sendMessage({ query: "getMatchRecords" }, function (queryResult) {
                matches = queryResult;
                if (matches.length === 0) {
                ...
                for (let mf = 0; mf < sizeNextGen - sizeTopParents; mf++) {
                    ...
                    do {
                        ...
                        if (mf < sizeTopParentsBreed * (ratioOrderedTopBestBreeding)) {
                            parent1 = sortingArray[0][0];
                            parent2 = sortingArray[mf][0];
                        }
                        else if (mf < sizeTopParentsBreed * (ratioEvenTopBestBreeding)) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    multiplierSlider.bind("input", function () {
        $("#multiplierField")[0].value = $("#multiplierSlider")[0].value;
    });
