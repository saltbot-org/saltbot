...
Simulator.prototype.evalMutations = function (mode) {
	...
	chrome.storage.local.get(["matches_v1", "characters_v1", "chromosomes_v1"], function (results) {
		...
		if (mode == "evolution" || mode == "mass") {
			...
			var accuracy = true;
			var unshackle = true;
			var weightAccToMoney = 0.99;//1 - 1/100000000000;			// valid range (0,1), enabled if accuracy & money are. 50% would be the original method. Also good for evening the magnitude between them.
